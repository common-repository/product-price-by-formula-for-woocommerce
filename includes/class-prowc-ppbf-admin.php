...
class ProWC_PPBF_Admin {
	...
	function __construct() {
		...
		$this->products_list_columns = array(
			'prowc_ppbf_enabled'   => __( 'Formula Enabled', 'product-price-by-formula-for-woocommerce' ),
			'prowc_ppbf_formula'   => __( 'Formula', 'product-price-by-formula-for-woocommerce' ),
			'prowc_ppbf_params'    => __( 'Params', 'product-price-by-formula-for-woocommerce' ),
			'prowc_ppbf_price'     => __( 'Price', 'product-price-by-formula-for-woocommerce' ),
		);
			...
			if ( prowc_ppbf()->core->is_price_by_formula_product( $_product ) ) {
				switch ( $column ) {
					...
					case 'prowc_ppbf_formula':
						echo esc_attr(prowc_ppbf()->core->get_product_formula( $_product ));
						break;
					case 'prowc_ppbf_params':
						...
						if ( ! empty( $params ) ) {
							echo esc_attr(implode( '<br>', $params ));
						}
						break;
					case 'prowc_ppbf_price':
						if ( $_product->is_type( 'variable' ) ) {
							...
							if ( ! empty( $prices ) ) {
								$from = min( $prices );
								$to   = max( $prices );
								echo wp_kses_post( $from !== $to ? wc_format_price_range( $from, $to ) : wc_price( $from ) );
							}
						} else {
							echo wp_kses_post(wc_price( prowc_ppbf()->core->price_by_formula( $_product->get_price(), $_product, true ) ));
						}
						break;
				}
			}
	...
	function output_dashboard_widget() {
		...
		$html .= '<input type="submit" name="prowc_ppbf_save_dashboard_widget_settings" class="button button-primary" value="' .
			__( 'Save', 'product-price-by-formula-for-woocommerce' ) . '">';
		$html .= '</p>';
		$html .= '</form>';
		echo $html;
	}
	...
	function add_final_price_preview_to_meta_box() {
		...
		if ( prowc_ppbf()->core->is_price_by_formula_product( $_product ) ) {
			...
			echo '<p style="border: 1px solid gray; padding: 10px;">' . 
				'<strong>' . esc_html__('Final price preview:', 'product-price-by-formula-for-woocommerce') . '</strong> ' . 
				sprintf(wc_price($_price)) . '</p>';
		}
	}
