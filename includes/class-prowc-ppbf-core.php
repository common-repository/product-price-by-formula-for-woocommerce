		...
		try {
			...
			if ( $output_errors ) {
				echo '<p style="color: red; border: 1px solid gray; padding: 10px;">' .
					sprintf(
					/* translators: 1: price formula, 2: error message */
					esc_html__( '<strong>Error in %1$s formula:</strong> %2$s', 'product-price-by-formula-for-woocommerce' ),
						'<code>' . esc_html( $_formula ) . '</code>', esc_html( $e->getMessage() ) ) . '</p>';	
			}
		}
	}
